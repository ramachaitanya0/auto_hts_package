...
class Auto_HTS_Regressor():
    def __init__(self, hier : OrderedDict()) -> None:
        self.hier = hier
        self.tree = None
        self.sum_mat = None
        self.sum_mat_labels = None
        self.list_ARIMA : list  = None
        self.list_HWSE  : list  = None
        self.list_PROP  : list  = None
        self.fit_dataframe : pd.DataFrame = None
        self.exogenus_variables : list = None
        self.predictable_variables : list = None
        

    def train_test_data(self,df: pd.DataFrame):
    ...
    def define_tree(self,df: pd.DataFrame, hier : OrderedDict()):
        """This Function returns key variables for forecasting  hierarchial time series  like summarising matrix, summarisnig matrix lables and tree structure.

        Args :
            df (Pandas DataFrame) : its the train data derived in the train_test_data function 
        ...
        """
        tree = hts.hierarchy.HierarchyTree.from_nodes(hier, df, root='total')
        sum_mat, _ = hts.functions.to_sum_mat(tree)
        sum_mat_labels = df.columns
        self.tree = tree
        self.sum_mat =sum_mat
        self.sum_mat_labels = sum_mat_labels
        return tree, sum_mat, sum_mat_labels

    def time_series_scores(self,forecast, actual):
    ...
    def forecast_models(self,temp_train, temp_test, tree, sum_mat, sum_mat_labels, exogenus_variables,m):
        ...
        for col in revised_forecasts_ARIM.columns:
            ...
            mape_ARIM.loc[0, col], rmse_ARIM.loc[0, col], mae_ARIM.loc[0, col], first_wk_mape_ARIM.loc[0, col] = self.time_series_scores(revised_forecasts_ARIM[col], temp_test[col])   
        ...
        for col in forecasts_HWSE.columns:
            ...
            mape_HWSE.loc[0, col], rmse_HWSE.loc[0, col], mae_HWSE.loc[0, col], first_wk_mape_HWSE.loc[
                0, col] = self.time_series_scores(revised_forecasts_HWSE[col], temp_test[col])
        ...
        for col in forecasts_PROP.columns:
            ...
            mape_PROP.loc[0, col], rmse_PROP.loc[0, col], mae_PROP.loc[0, col], first_wk_mape_PROP.loc[
                0, col] = self.time_series_scores(revised_forecasts_PROP[col], temp_test[col])
    ...
    def ensemble_fcst(self,temp_test, revised_forecasts_ARIM, revised_forecasts_HWSE, revised_forecasts_PROP,  mape_ARIM,
    ...
    def forecast_refit(self,ts_data_2, list_ARIMA,  list_HWSE,  list_PROP, tree, sum_mat, sum_mat_labels, fcst_input_data,exogenus_variables):
    ...
    def forecast_output(self,final_forecasts_ARIM, final_forecasts_HWSE, final_forecasts_PROP, sum_mat, sum_mat_labels, fcst_input_data):
    ...
    def fit(self,df:pd.DataFrame,exogenus_variables : list , predictable_variables : list , m :int) -> None:
        self.fit_dataframe = df
        self.exogenus_variables = exogenus_variables
        self.predictable_variables = predictable_variables
        train_data,test_data =    self.train_test_data(df)
        tree, sum_mat, sum_mat_labels =  self.define_tree(train_data[predictable_variables],self.hier) 
        revised_forecasts_ARIM, revised_forecasts_HWSE, revised_forecasts_PROP,  mape_ARIM, mape_HWSE, mape_PROP  = \
    self.forecast_models(train_data, test_data, tree, sum_mat, sum_mat_labels, exogenus_variables, m)
        list_ARIMA, list_HWSE, list_PROP =  self.ensemble_fcst(test_data, revised_forecasts_ARIM, revised_forecasts_HWSE, revised_forecasts_PROP,  mape_ARIM,
                  mape_HWSE, mape_PROP, sum_mat_labels)
        self.list_ARIMA = list_ARIMA
        self.list_HWSE = list_HWSE
        self.list_PROP = list_PROP

    def predict(self,df:pd.DataFrame):
        final_forecasts_ARIM, final_forecasts_HWSE, final_forecasts_PROP = self.forecast_refit(self.fit_dataframe, self.list_ARIMA,self.list_HWSE,self.list_PROP,self.tree,self.sum_mat, self.sum_mat_labels,df,self.exogenus_variables )
        revised_forecast =  self.forecast_output(final_forecasts_ARIM, final_forecasts_HWSE, final_forecasts_PROP, self.sum_mat, self.sum_mat_labels, df) 
